...
import com.bmwcarit.barefoot.matcher.ServerControl;
...
public class ServerTest {
    ...
    private void sendRequest(InetAddress host, int port, List<MatcherSample> samples)
        ...
        KState<MatcherCandidate, MatcherTransition, MatcherSample> state =
                new KState<MatcherCandidate, MatcherTransition, MatcherSample>(new JSONObject(response),
                        new MatcherFactory(ServerControl.getServer().map));

        assertEquals(samples.size(), state.sequence().size());

        for (MatcherCandidate candidate : state.sequence()) {
            candidate.point().edge().geometry();
        }
